...
public class MoveBackward implements Action {
	...
	public MoveBackward(SceneNode dolphinN) {
	...
	public void performAction(float time, Event e) {
		...
		Vector3f p = (Vector3f) dolphinN.getLocalPosition(); //local position dolphin
		//Vector3f camPos = (Vector3f) dolphinCam.getLocalPosition(); //position of camera 
		Vector3f p1 = (Vector3f) Vector3f.createFrom(-.1f * n.x(), -.1f * n.y(), -.1f * n.z()); //new position to go in
		Vector3f p2 = (Vector3f) p.add((Vector3) p1); //new position of dolphin
